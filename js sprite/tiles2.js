...
TileMap.prototype.draw = function()
{
    ...
    if (arguments.length == 5)
    {
        cx = 0;
        cy = 0;
        cw = this.width * tileSet.tileWidth;
        ch = this.height * tileSet.tileHeight;
        dx = arguments[1];
        dy = arguments[2];
        dw = arguments[3];
        dh = arguments[4];
    ...
    for (var z in this.tiles)
    {
        ...
        for (var yy=Math.floor(this.viewport.y),dy=Math.ceil(this.viewport.y + this.viewport.height); yy<dy; ++yy)
        {
            var line = layer[yy];
